...
async function searchValue(status) {
    ...
    await fetch('https://dev-m7g7iotnx386bhq.api.raw-labs.com/json-programming-heroes')
        .then(response => response.json())
        .then(valueSet => {
          
            findsize(valueSet.resume)

            if(searchval.length==0) {
                
                controller()
                updatePage(valueSet.resume)
            }
            else{
                var valueSet = Object.values(valueSet.resume).filter(value => {
